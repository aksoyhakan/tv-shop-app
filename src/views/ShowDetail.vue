<template>
  ...
  <div v-if="selectedShow" class="show-details">
    ...
    <div class="show-header">
      <img :src="selectedShow.image?.original || defaultImage" alt="Show Image" />
      <div class="show-info">
        ...
        <ul class="show-extra-details">
          ...
          <li><strong>Runtime:</strong> {{ selectedShow.runtime ? selectedShow.runtime + ' mins' : 'N/A' }}</li>
          <li><strong>Premiered:</strong> {{ selectedShow.premiered || 'Unknown' }}</li>
          <li><strong>Ended:</strong> {{ selectedShow.ended || 'Ongoing' }}</li>
          <li><strong>Genres:</strong> {{ selectedShow.genres.join(', ') }}</li>
          <li><strong>Average Rating:</strong> {{ selectedShow.rating.average || 'No Rating' }}</li>
        </ul>
    ...
    <div class="episode-list">
      <div v-for="episode in episodes" :key="episode.id" class="episode-card">
        <img :src="episode.image?.medium || defaultEpisodeImage" alt="Episode Image" />
...
export default {
  setup() {
    ...
    const defaultEpisodeImage = 'https://via.placeholder.com/150x90?text=No+Image';
...
.episode-card {
  min-width: 300px; /* Sabit genişlik */
